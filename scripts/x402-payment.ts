...
import type { McpAdapterConfig } from "../src/config.js";
...
function buildMaxPaymentPolicy(maxPayment: bigint): PaymentPolicy {
  return (_x402Version, paymentRequirements) =>
    paymentRequirements.filter((requirement) => {
      const amount = "maxAmountRequired" in requirement ? (requirement as { maxAmountRequired: string }).maxAmountRequired : (requirement as { amount: string }).amount;
      try {
